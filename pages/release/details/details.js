// pages/release/details/details.js
      ...
      request('book.post.share', {
        ...
        if (response.data.success == true) {
          ...
          setTimeout(function () {
            wx.hideLoading()
            wx.redirectTo({
              url: '/pages/books/companionshipbooks/companionshipbooks?uid=' + response.data.uid
            })
          }, 1000)
        } else {
          wx.showLoading({
  ...
  onLoad: function (options) {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        that.setData({
          init_price: res.data.init_price/100
        })
        that.setData({
          recommend_price: (that.data.init_price*0.4).toFixed(2)
        })
      }
    })
    wx.getLocation({
